...
vi.mock("@/lib/queries", () => ({
  getFaqs: async () => [
    { id: 1, question: "Who is Agastya?", answer: "A software engineer.", order: 0 },
  ],
}));
...
describe("llms.txt", () => {
  ...
  it("contains the identity section", async () => {
    ...
    expect(text).toContain("AGASTYA12343534");
  ...
  it("contains GitHub link", async () => {
    const res = await GET();
    const text = await res.text();
    expect(text).toContain("github.com/AGASTYA12343534");
  });

  it("contains at least one FAQ question", async () => {
    const res = await GET();
    const text = await res.text();
    expect(text).toContain("Who is Agastya?");
  });
});
