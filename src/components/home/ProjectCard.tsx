...
export function ProjectCard({ project }: { project: MergedProject }) {
  ...
  const githubUrl = project.htmlUrl ?? `https://github.com/AGASTYA12343534/${project.repo}`;
        ...
        {(project.language || project.tags.length > 0) && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {project.language && (
              ...
              <span className="rounded-full border border-(--brand-violet)/40 bg-(--chip-surface) px-2 py-0.5 text-xs font-semibold text-(--accent-on-chip)">
                {project.language}
              </span>
            )}
            {project.tags.map((t) => (
