...
describe("buildTestimonialEmail", () => {
  it("escapes html and embeds an approve link with the id", () => {
    const { subject, html } = buildTestimonialEmail(input, "https://AGASTYA12343534.com");
    expect(subject).toContain("Jane");
    expect(html).toContain("Jane &lt;b&gt;Doe&lt;/b&gt;");
    expect(html).toContain("dedication");
    expect(html).toContain("https://AGASTYA12343534.com/admin");
...
describe("notifyNewTestimonial", () => {
  beforeEach(() => {
    ...
    process.env.RESEND_FROM_EMAIL = "Portfolio <noreply@AGASTYA12343534.com>";
    process.env.NEXT_PUBLIC_SITE_URL = "https://AGASTYA12343534.com";
  });
  afterEach(() => {
  ...
  it("sends when configured, using the env from-address", async () => {
    ...
    expect(sendMock.mock.calls[0]![0]).toMatchObject({
      from: "Portfolio <noreply@AGASTYA12343534.com>",
      to: "me@example.com",
    });
  });
