...
function githubRepo(name: string, stars: number) {
  return {
    name,
    full_name: `AGASTYA12343534/${name}`,
    html_url: `https://github.com/AGASTYA12343534/${name}`,
...
describe("getCachedRepoData", () => {
  it("returns cached data for fresh rows (<24h) without calling getRepo", async () => {
    ...
    const result = await getCachedRepoData(["AGASTYA12343534/my-repo"]);

    expect(mockGetRepo).not.toHaveBeenCalled();
    expect(result.get("AGASTYA12343534/my-repo")?.stars).toBe(50);
  });
  ...
  it("fetches from GitHub and upserts for stale rows (>24h)", async () => {
    ...
    expect(mockGetRepo).toHaveBeenCalledWith("AGASTYA12343534/my-repo");
    expect(mockInsert).toHaveBeenCalled();
    expect(result.get("AGASTYA12343534/my-repo")?.stars).toBe(120);
  });
  ...
  it("fetches from GitHub for missing rows (no cache)", async () => {
    ...
    const result = await getCachedRepoData(["AGASTYA12343534/new-repo"]);

    expect(mockGetRepo).toHaveBeenCalledWith("AGASTYA12343534/new-repo");
    expect(result.get("AGASTYA12343534/new-repo")?.stars).toBe(99);
  });
  ...
  it("falls back to stale cache when GitHub fetch throws (resilience)", async () => {
    ...
    expect(result.get("AGASTYA12343534/my-repo")?.stars).toBe(77);
  ...
  it("falls back to zero stats when GitHub fails and no cache exists", async () => {
    ...
    const result = await getCachedRepoData(["AGASTYA12343534/ghost-repo"]);

    const stats = result.get("AGASTYA12343534/ghost-repo");
  ...
  it("processes multiple slugs, fetching only stale ones", async () => {
    ...
    await getCachedRepoData(["AGASTYA12343534/fresh-repo", "AGASTYA12343534/stale-repo"]);

    // Only the stale one gets fetched
    expect(mockGetRepo).toHaveBeenCalledTimes(1);
    expect(mockGetRepo).toHaveBeenCalledWith("AGASTYA12343534/stale-repo");
  });
});
